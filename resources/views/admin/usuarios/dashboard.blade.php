            ...
            <div class="modulo-footer">
                @if($moduloCompleto)
                    @php
                        ...
                        $quizAprobado   = $resultadoQuiz && $resultadoQuiz->total > 0
                            && ($resultadoQuiz->correctas / $resultadoQuiz->total) >= 0.7;
                        $moduloTotalCompleto = !$tienePreguntas || $quizAprobado;
                    @endphp

                    @if($moduloTotalCompleto)
                        {{-- Lecciones completas + quiz aprobado (o sin quiz) --}}
                        <div style="display:flex;align-items:center;gap:.8rem;flex-wrap:wrap">
                            <span class="badge-completado">🏆 Módulo completado</span>
                            @if($tienePreguntas)
                                <a href="{{ route('quiz.show', $modulo->id) }}"
                                   style="display:inline-flex;align-items:center;gap:6px;padding:.4rem 1rem;background:#EFF6FF;color:#1E3A8A;border-radius:20px;font-size:.8rem;font-weight:700;text-decoration:none;border:1.5px solid #BFDBFE">
                                    🔄 Reintentar Quiz
                                </a>
                            @endif
                        </div>
                    @else
                        {{-- Lecciones completas pero quiz pendiente o reprobado --}}
                        <div style="display:flex;align-items:center;gap:.8rem;flex-wrap:wrap">
                            <span style="display:inline-flex;align-items:center;gap:6px;font-size:.82rem;font-weight:600;color:#92400E;background:#FEF3C7;padding:.4rem 1rem;border-radius:20px;border:1.5px solid #FDE68A">
                                📚 Lecciones completas
                            </span>
                            @if($tienePreguntas)
                                @if($resultadoQuiz)
                                    {{-- Ya intentó pero no aprobó --}}
                                    <span style="font-size:.8rem;color:#6B7280">
                                        Último intento: {{ round($resultadoQuiz->correctas/$resultadoQuiz->total*100) }}%
                                        — necesitas ≥70%
                                    </span>
                                    <a href="{{ route('quiz.show', $modulo->id) }}"
                                       style="display:inline-flex;align-items:center;gap:6px;padding:.4rem 1.1rem;background:#EF4444;color:#fff;border-radius:20px;font-size:.82rem;font-weight:700;text-decoration:none">
                                        🔄 Reintentar Quiz
                                    </a>
                                @else
                                    {{-- No ha hecho el quiz --}}
                                    <a href="{{ route('quiz.show', $modulo->id) }}"
                                       style="display:inline-flex;align-items:center;gap:6px;padding:.45rem 1.2rem;background:#15803D;color:#fff;border-radius:20px;font-size:.82rem;font-weight:700;text-decoration:none">
                                        🎯 Hacer Quiz Final
                                    </a>
                                @endif
                            @endif
