...
<head>
    ...
    <title>Bitlyx — Nueva Lección</title>
...
<div class="app-wrapper">
    ...
    <main class="main-content">
        <div class="page-header">
            <div>
                <h1 class="page-title"><i class="fas fa-plus-circle" style="color:#4ADE80;margin-right:10px"></i>Nueva Lección</h1>
                <p class="page-sub">Módulo: {{ $modulo->titulo }}</p>
            </div>
            <div style="display:flex;gap:10px;align-items:center">
                <button class="dark-btn" id="dark-btn" title="Modo oscuro">🌙</button>
                <a href="/modulos/{{ $modulo->id }}/lecciones" class="btn-back">← Volver</a>
            </div>
        </div>
        <div class="form-card">
            <form action="/modulos/{{ $modulo->id }}/lecciones" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="titulo" class="form-input" required maxlength="150"
                           value="{{ old('titulo') }}" placeholder="Ej: Variables en PHP">
                </div>
                <div class="form-group">
                    <label class="form-label"><i class="fas fa-align-left"></i> Contenido</label>
                    <textarea name="contenido" class="form-textarea" rows="6"
                              placeholder="Contenido de la lección...">{{ old('contenido') }}</textarea>
                </div>
                <div class="form-group">
                    <label class="form-label"><i class="fas fa-sort-numeric-up"></i> Orden</label>
                    <input type="number" name="orden" class="form-input" value="{{ old('orden', 1) }}" min="1" style="max-width:120px">
                </div>
                <div class="form-actions">
                    <button type="submit" class="btn-primary"><i class="fas fa-save"></i> Guardar Lección</button>
                    <a href="/modulos/{{ $modulo->id }}/lecciones" class="btn-secondary"><i class="fas fa-times"></i> Cancelar</a>
                </div>
            </form>
        </div>
    </main>
